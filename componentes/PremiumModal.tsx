...
const PremiumModal = ({ onClose }: PremiumModalProps) => {
  ...
  const handleActivate = () => {
    // Configuração de Billing: ID ultra_ia_monthly_subscription
    // Em um app real, aqui chamaria o StoreKit/GoogleBilling
    upgradePlan();
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-[2rem] overflow-hidden shadow-[0_0_60px_rgba(6,182,212,0.4)] animate-in zoom-in-95 duration-300 border-[3px] border-cyan-400">
        
        {/* Botão Fechar */}
        <button 
          onClick={onClose}
          className="absolute top-4 right-4 z-10 p-2 bg-gray-50 rounded-full hover:bg-gray-100 transition text-gray-400 hover:text-gray-600"
        >
          <X size={20} />
        </button>

        {/* Hero Section */}
        <div className="bg-white p-8 text-center relative overflow-hidden pb-2">
          {/* Efeito Neon Topo */}
          ...
          <div className="relative z-10 pt-4">
            <div className="mx-auto w-24 h-24 bg-cyan-50 rounded-full flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(6,182,212,0.3)] border-2 border-cyan-100 animate-bounce-slow">
            ...
            <p className="text-slate-500 font-semibold text-sm uppercase tracking-wider">Você no Controle Total</p>
          </div>
        </div>
        ...
        <div className="px-8 pb-8 space-y-4">
          ...
          <div className="pt-6 border-t border-gray-100 text-center">
            ...
            <p className="text-xs text-slate-400 font-medium">
              Depois apenas <strong className="text-slate-600">R$ 17/mês</strong>. Cancele quando quiser.
            </p>
          </div>
        </div>
...
const FeatureItem = ({ icon, text }: { icon: React.ReactNode, text: string }) => (
    <div className="flex items-center gap-3 bg-slate-50 p-3.5 rounded-xl border border-slate-100 transition-colors hover:border-cyan-200 hover:bg-cyan-50/30">
        <div className="text-cyan-500 bg-white p-1.5 rounded-lg shadow-sm">{icon}</div>
